...
class Meks_ESS {
    ...
    public function get_inline_styles() {
        ...
        if ( $this->options['color']['type'] == 'custom' ) {
            ...
            $color = esc_attr( $this->options['color']['custom_color'] );
...
?>
    ...
    private function sanitize_field( $value, $type ) {
        ...
        switch ($type) {
            ...
            case 'text':
                ...
                if ( is_array( $value ) ) {
                    ...
                    foreach ( $value as $key => $val ) {
                        $new_input[$key] = isset( $value[$key] )  ? sanitize_text_field( $val ) : '';
                    }
            ...
            case 'color':
                $new_input = array();
                $new_input['type'] = sanitize_key( $value['type'] );
                $new_input['custom_color'] = sanitize_hex_color( $value['custom_color'] );
                return $new_input;
                break;
            ...
            default:
                    ...
                    foreach ( $value as $key => $val ) {
                        $new_input[$key] = isset( $value[$key] )  ? sanitize_key( $val ) : '';
                    }
...
?>
        ...
        <?php
        printf( '<div class="platforms-note">%s</div>', esc_html__( 'Note: To reorder platforms just click, hold, and drag them.', 'meks-easy-social-share' ) );
    }
    ...
    public function print_variant_field( $args ) {
        ...
        foreach ( $this->styles['variant'] as $key => $value ) {
            printf(
                ...
                esc_attr($key),
                checked( $args, $key, false ),
                esc_url(MEKS_ESS_URL . 'assets/images/variant-'.$key.'.svg'),
                esc_attr($key),
    ...
    public function print_color_field( $args ) {
        ...
        printf(
            ...
            esc_html__( 'Brand' , 'meks-easy-social-share' )
        );
    ...
    public function print_location_field( $args ) {
        ...
        printf(
            ...
            esc_html__( 'Above content', 'meks-easy-social-share' )
        );
        printf(
            ...
            esc_html__( 'Below content', 'meks-easy-social-share' )
        );
        printf(
            ...
            esc_html__( 'Above and below content', 'meks-easy-social-share' )
        );
        printf(
            ...
            esc_html__( 'Custom (template tag) use:', 'meks-easy-social-share' ) . ' <code> meks_ess_share(); </code>'
        );
    ...
    public function print_label_share_field( $args ) {
        ...
        printf(
            ...
            esc_html__( 'Enabled', 'meks-easy-social-share' )
        );
    ...
    public function parse_settings_for_output() {
        ...
        $before = '<div class="meks_ess layout-'.esc_attr($this->options['style']).'-'.esc_attr($this->options['variant']).' '.esc_attr($this->styles['style'][$this->options['style']]).' '.esc_attr($this->styles['variant'][$this->options['variant']]).'">';
        $after = '</div>';
        ...
        if (  !empty( $share_label ) ) {
            $before = '<div class="meks_ess_share_label"><h5>' . esc_html( $share_label ) . '</h5></div>'. $before;
        }
